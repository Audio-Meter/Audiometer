//
//  DrwaingViewController.swift
//  Audiometer
//
//  Created by Dogra Tech on 31/03/20.
       ...
       // var tutorialView: GPBTutorialView!
        ...
        override func viewDidLoad() {
           ...
           // tap.numberOfTapsRequired = 2
            ...
            self.navigationItem.rightBarButtonItem = UIBarButtonItem(title: "Next", style: .done, target: self, action: #selector(action))

            
            //tutorialView = GPBTutorialView.instanceFromNib() as? GPBTutorialView
//            tutorialView.frame = CGRect(      x:0, y:0, width:self.view.frame.wid th, height:self.view.frame.height)
           // self.view.addSubview(tutorialView)
        }
        @objc func action(sender: UIBarButtonItem) {
         
            let navigation = self.storyboard?.instantiateViewController(withIdentifier: "MiniCogThirdStepViewController") as! MiniCogThirdStepViewController
                               
                   
            navigation.FirstStepCompiltionDetails = FirstStepCompiltionDetails
            
            navigation.DrawnImagez = mainImageView.image
